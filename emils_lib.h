...
namespace el {
	...
	namespace StrCalc {
		...
		int getThats(std::string string, char a, int place)
		{
			...
			else for (int i = 0; i < glos(string); i++) if (string[i] == a) if (place == 0) { result = i; break; } else place--;
			return result;
		}

		std::string getBetweenThat(std::string string, int p1, int p2) { std::string result = "";
	...
	namespace Strings {
		...
		template<int length> arrayString<length> split(std::string wllSplit, char c) {
			...
			for (int i = 0; i < length; i++) if (wllSplit[i] == c) { counter++; continue; } else wllReturn.a[counter] += wllSplit[i];
			return wllReturn;
		}
	...
	namespace Geo {
		typedef int angle;
		struct Angle2 { Vectors::Vector2 v; angle angle; }; Angle2 getAngle(float x, float y, angle angleD) { return { Vectors::get(x,y),angleD }; }
		bool operator==(Angle2 a, Angle2 b) { return a.v.x == b.v.x && a.v.y == b.v.y && a.angle == b.angle; }

		struct Triangle2 { Angle2 a, b, c; }; Triangle2 getTriangle(Angle2 a, Angle2 b, Angle2 c) { return { a,b,c }; }

		Angle2 getDiffAngle(Triangle2 t1, Triangle2 t2) {
			Angle2 t1Angle[3] = { t1.a,t1.b,t1.c }; Angle2 t2Angle[3] = { t2.a,t2.b,t2.c };
			Angle2 result = { Vectors::get(0,0), 0 };
...
#define JJNumber 0.4
	namespace Physics {
		...
		struct Matter { Vectors::Vector2 v; int charge; }; Matter get(Vectors::Vector2 v, int charge) { return { v,charge }; }
